...
namespace TDSTK{
	...
	public class Ability : Item{
		...
		public AttackStats GetRuntimeAttackStats(){
			...
			AttackStats stats=aStats.Clone();

			stats.damageMin*=(1+perk.GetAbilityDamageMul(ID));
			stats.damageMax*=(1+perk.GetAbilityDamageMul(ID));
			
			stats.critChance*=(1+perk.GetAbilityCritMul(ID));
			stats.critMultiplier*=(1+perk.GetAbilityCritMulMul(ID));
			
			stats.aoeRadius*=(1+perk.GetAbilityAOEMul(ID));
