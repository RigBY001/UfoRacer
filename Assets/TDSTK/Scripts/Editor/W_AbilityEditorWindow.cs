...
namespace TDSTK {
	...
	public class AbilityEditorWindow : TDSEditorWindow {
		...
		Vector2 DrawAbilityConfigurator(float startX, float startY, Ability ability){
			...
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			ability.ShopCost=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, 40, height), ability.ShopCost);
			...
			if(ability.type==_AbilityType.Movement){
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				if(ability.moveType==_MoveType.Dash) ability.duration=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), ability.duration);
