...
namespace TDSTK{
	...
	public class ShootObject : MonoBehaviour {
		...
		void Update () {
			...
			if(type==_SOType.Homing){
				if(targetUnit!=null){
					...
					if(Time.time-shootTime>trackingDuration){
						targetUnit=null;
						targetPos+=new Vector3(Random.value-0.5f, 0, Random.value-0.5f)*2*spread/2;
					}
				}
		...
		void ShootEffect(Vector3 pos, Quaternion rot, Transform shootPoint){
			if(shootEffect==null) return;
			
			GameObject effectInstance=null;
			
			if(!destroyShootEffect) effectInstance=(GameObject)ObjectPoolManager.Spawn(shootEffect, pos, rot);
			else effectInstance=(GameObject)ObjectPoolManager.Spawn(shootEffect, pos, rot, destroyShootDuration);
			
			effectInstance.transform.parent=shootPoint;
		}
