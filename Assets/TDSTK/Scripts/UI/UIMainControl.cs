			...
			*/
			...
			if(GameControl.IsGamePlaying()){
				...
				if(player!=null && !player.IsDestroyed() && Input.touchCount==0){
					...
					// }
					// else player.AimTurretMouse(Input.mousePosition); // OLD WORK CODE

					if(player.enableAutoAim) player.AutoAim();

					//fire
					// bool continousFire=player.ContinousFire() & (Input.GetMouseButton(0) || Input.GetButton("Fire1"));
					// if(Input.GetMouseButtonDown(0) || Input.GetButtonDown("Fire1") || continousFire) player.FireWeapon(); // OLD WORK CODE
					// if(player.ContinousFire()) player.FireWeapon();
