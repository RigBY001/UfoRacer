...
namespace TDSTK{
	...
	public class UnitPlayer : Unit {
		...
		void Init(){
			...
			if(!weaponInitiated){
				...
				for(int i=0; i<weaponList.Count; i++){
					GameObject obj=MountWeapon(Instantiate(weaponList[i].gameObject));
		...
		public void AutoAim(){
			...
			Weapon weapon = weaponList[weaponID];
			float range = weapon.GetRange();
			if(Vector3.Distance(thisT.position,target.thisT.position) > range) return;
			if(NavMesh.Raycast(thisT.position, target.thisT.position, out NavMeshHit hit, NavMesh.AllAreas)) return;

			// Vector3 p1=turretObj.position;		p1.y=0;
			Vector3 p1=turretObj.position;		

			// Vector3 p2=target.thisT.position;	p2.y=0;
			Vector3 p2=target.thisT.position;
			
			Quaternion wantedRot=Quaternion.LookRotation(p2-p1);
			if(!smoothTurretRotation) turretObj.rotation=wantedRot;
			else turretObj.rotation=Quaternion.Slerp(turretObj.rotation, wantedRot, Time.deltaTime*10);
			FireWeapon();
			return;
		}
		...
		public void FireWeapon(){
			...
			if(fireState==0){
				...
				ShootObject.AimInfo aimInfo = null;
		
				if(enableAutoAim) {
					
					Unit target=UnitTracker.GetNearestUnit(thisT.position);
					
					if (target != null) aimInfo = new(target);
                }
				if(aimInfo == null) StartCoroutine(ShootRoutine());
				else StartCoroutine(ShootRoutine(aimInfo));
		...
		public bool CheckObstacle(Vector3 dir){
			...
			LayerMask mask = 1 << 0;
			bool flag1=Physics.Raycast(pos+dirSide, dir, collider.radius*1.1f, mask);
			bool flag2=Physics.Raycast(pos-dirSide, dir, collider.radius*1.1f, mask);
