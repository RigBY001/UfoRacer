...
namespace TDSTK{
	...
	public class UnitSpawner : MonoBehaviour {
		...
		private List<Unit> spawnedUnit = new List<Unit>();
		...
		void Start(){
			...
			Garage.OnEnterGarge += StopSpawnAndClear;
			Garage.OnExitGarage += ActivateSpawner;
		}
		private void ActivateSpawner(){
			IsActive = true;
		}
		private void StopSpawnAndClear(){
			IsActive = false;
			ClearUnit();
		}
		...
		IEnumerator SpawnFreeForm(){
			...
			while(true){
				...
				if(spawnAroundPlayer){
					UnitPlayer unitPlayer = GameControl.GetPlayer();
					while(unitPlayer == null) yield return null;
					if(unitPlayer.IsDestroyed()) yield return null;
					while(!RandomPoint(unitPlayer.gameObject.transform.position,out pos))yield return null;
				}
				else{
					pos =spawnAreaList[rand].GetPosition();
				}
				...
				// }else pos =spawnAreaList[rand].GetPosition();
